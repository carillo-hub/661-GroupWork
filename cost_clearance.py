...
global FinalStateID
FinalStateID = tuple(
    FinalState)  # pixel coors                                #will be referenced for checking if it reached the goal
...
while ID != FinalStateID:  # will become "while explored node != G"              #pixel coors check if current node == final state                                                    #While loop until FinalState is reached
    ...
    while i <= (len(VisitedQ_eachRound)):  # for each node in OPEN LIST
        ...
        # outputVideo.release()
        # sys.exit()
        while cost_map[VisitedQ_eachRound[min][0], VisitedQ_eachRound[min][1]] < cost_map[
            VisitedQ_eachRound[i][0], VisitedQ_eachRound[i][1]]:  # choose lowest cost
            i += 1
        if cost_map[VisitedQ_eachRound[min][0], VisitedQ_eachRound[min][1]] > cost_map[
            VisitedQ_eachRound[i][0], VisitedQ_eachRound[i][1]]:  # choose lowest cost
        ...
        Checkit = VisitedQ_eachRound.pop(
            min)  # remove lowest costing node from OPEN LIST and explore it                               #answer NO, so remove FIFO node from Q and store in a variable
        VisitedQ.enqueue(Checkit)  # add this node to the CLOSED list
        ID = tuple(
            Checkit)  # tuple pixel coors                                                   #Get the tuple value of the node in variable
