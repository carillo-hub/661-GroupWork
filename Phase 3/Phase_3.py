...
ctr = 0
VisitedDict = {}
radius = 5 #0.038   # robot radius in pixels taken from the data sheet
L = 2  #0.354        #wheel distance L taken from the data sheet
...
fps = 50
outputVideo = cv2.VideoWriter("Sweeping_new_4.mp4", fourcc, fps,(xscale, yscale))  # output resolution must match input frame (60% resized from 1980x1020)
print("\n...Creating Video...\n")
...
def Step1(NewNode, theta_i, UL, UR, ParentNode):
    ...
    while t < 1:
        ...
        D = D + math.sqrt(math.pow((0.5*radius*(UL+UR)*math.cos(theta_n)*dt),2) + math.pow((0.5*radius*(UL+UR)*math.sin(theta_n)*dt),2))    
    ...
    x_raw = (2 * int(round(x_step)))
    x_new = x_raw / 2  # applies the 0.5 threshold
    y_raw = (2 * int(round(y_step)))
...
VisitedDict[tuple(initial_state[:2])] = 'Initial State'                                 # add initial state into Visited Data Structure
...
while ID != FinalStateID:             
    ...
    ID = tuple(Checkit[:2])                                     # tuple pixel coors
