...
def Plot_OurMap(OurMap):
    ...
    for EachID in OurMap:
        ...
        if t%5 == 0:
            ...
            start_node_cpy[0] = CurrentNode[0]
    ...
    print("\nDone. Can see results in NodePath.txt and Sweeping4.mp4!")
    sys.exit()
...
def CheckAction(NewNode, action_cost, CTG, n_c):
    global FinalStateID
    nc_new = NewNode.copy()
    ...
    if flag_obc == False:
        for z in range(len(closed_lis)):
            if closed_lis[z][0] == NewNode[0] and closed_lis[z][1] == NewNode[1]:
                fl = True
                break
        ...
        if fl == False:
            if ID in VisitedDict and cost_map[int((ID[0]) / threshold), int((ID[
                ...
                VisitedDict[ID] = parent_ID  # add to visited Dictionary the child/parent
                cost_map[
                    int((ID[0]) / threshold), int((ID[1]) / threshold)] = total_cost  # ensure lowest cost is stored on cost map
                ctc_map[int((ID[0]) / threshold), int((ID[1]) / threshold)] = CTC
                if ctr % 1 == 0:
                    cv2.arrowedLine(mask, (int(n_c[0]), int(n_c[1])), (int(nc_new[0]), int(nc_new[1])), (0,0,255), 1, tipLength=0.5)
                    cv2.imshow("arrow xp", mask)
                    cv2.waitKey(1)
                    outputVideo.write(mask)
            # print("curr cost", new_cost)


# WILL NOT USE FOR PHASE 2
def MoveUp(NewNode):
    x, y = NewNode  # check the action                           #array value at the blank ("0")
    swap1 = [x, y - 1]  # perform the action                     #array value to left of blank
    NewNode = swap1
    ctc = 1  # incremental cost for the action
...
def MoveUpRight(NewNode):
    ...
    NewNode = swap1
    ctc = 1.414  # incremental cost for the action
    ctg = ((NewNode[0] - FinalState[0]) ** 2 + (NewNode[1] - FinalState[1]) ** 2) ** 0.5  # euclidian distance CTG
    CheckAction(NewNode, ctc, ctg)  # check the action
...
def Step1(NewNode, n_c):
    ...
    ctg = ((NewNode[0] - FinalState[0]) ** 2 + (NewNode[1] - FinalState[1]) ** 2) ** 0.5  # euclidian distance CTG
    CheckAction(NewNode, ctc, ctg, n_c)  # check the action


def Step2(NewNode, n_c):
    x, y = NewNode  # unpack the node into pixel x y coords
    Xtheta = 2 * theta
    x_step = step * math.cos(Xtheta + theta_adaptive)
    y_step = step * math.sin(Xtheta + theta_adaptive)
...
cost_map = np.inf * np.ones(
    (int(400 / threshold), int(300 / threshold)))  # initialize cost map where each node = infinity
